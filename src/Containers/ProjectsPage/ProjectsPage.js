...
class ProjectPage extends Component {
    ...
    render() {
        ...
        let projectCards = filteredProjects.map((projectKey, index) => {
            return <ProjectCard project={this.state.projects[projectKey]} key={index} select={() => this.selectProjectHandler(projectKey)} />
        })
        ...
        const popup = <ProjectPopup click={this.removeProjectPopup} selectedProject={this.state.selectedProject}/>


        return (
            <div data-test="component-project-card" className={classes.container}>
                <div className={classes.landing}>
                    <h1 className={classes.title}><span>Projects</span>_</h1>
