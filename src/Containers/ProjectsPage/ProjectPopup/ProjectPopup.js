...
const ProjectPopup = (props) => {
    ...
    if(props.selectedProject !== null) {
        project = (
            <div>
                ...
                <h1 >{data.title}</h1>
                    <img src={process.env.PUBLIC_URL + '/images/cancel.png'} onClick={props.click}/>
                </div>
                
                <div className={classes.details}>
                    {data.fullDescription.map(para => <p>{para}</p>)}
                </div>
            </div>
    ...
    return (
        <div className={[classes.container, showPopup].join(' ')} >
            {project}
        </div>
    )
}
